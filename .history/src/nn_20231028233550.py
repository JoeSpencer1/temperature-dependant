...
def mfnn(data, lay = 2, wid = 128, aux = 8):
    x_dim, y_dim = 4, 1
    activation = "selu"
    initializer = "LeCun normal"
    regularization = ["l2", 0.01]
...
def validation_one(yname, trname, tstname, type, train_size, angles=[], lay = 3, wid = 32):
    ...
    print(mape)
    print(yname, 'validation_one ', trname, ' ', tstname, ' ', str(train_size), ' ', np.mean(mape), ' ', np.std(mape))
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + trname + ' ' +  tstname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')

def validation_two(yname, trname0, trname1, tstname, type, train_size, angles=[], lay = 3, wid = 32):
    ...
    if type == 'Berk':
        data = BerkovichDataT(yname)
    if type == 'Exp':
        data = ExpDataT(trname0, yname)
    data2 = ExpDataT(trname1, yname)
    tdata = ExpDataT(tstname, yname)
    ...
    if train_size[1] == 80:
        kf = RepeatedKFold(n_splits=5, n_repeats=2, random_state=0)
    elif train_size[1] == 90:
        kf = KFold(n_splits=10, shuffle=True, random_state=0)
    else:
        kf = ShuffleSplit(
            n_splits=10, test_size=len(data2.X) - train_size[1], random_state=0
        )
    iter = 0
    for train_index, test_index in kf.split(data2.X):
        iter += 1
        print('\nCross-validation iteration: {}'.format(iter))

        print(data2.X)
        X_train, X_test = data2.X[train_index], tdata.X[test_index]
        y_train, y_test = data2.y[train_index], tdata.y[test_index]
        ...
        '''
    print(mape)
    print(yname, 'validation_one ', str(trname0), ' ', tstname, ' ', str(train_size), ' ', np.mean(mape), ' ', np.std(mape))
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + str(trname0) + ' ' +  tstname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')
