...
def validation_one(yname, trname, tstrname, type, train_size, angles=[], lay = 3, wid = 32):
    ...
    print(mape)
    print(yname, 'validation_one ', trname, ' ', tstrname, ' ', str(train_size), ' ', np.mean(mape), ' ', np.std(mape))
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + trname + ' ' +  tstrname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')
